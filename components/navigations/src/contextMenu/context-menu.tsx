...
import { calculatePosition, isCollide, OffsetPosition, fit } from '@syncfusion/react-popups';
 ...
 */
export interface MenuAnimationProp {
    /**
...
export interface ContextMenuProps {
     ...
     * * event - Specifies the React synthetic event triggered by the user interaction.
     *
     * @event onSelect
     */
    onSelect?: (args: MenuEvent) => void;
}
...
export const ContextMenu: React.ForwardRefExoticComponent<ContextMenuComponentProps & React.RefAttributes<IContextMenu>> =
    forwardRef<IContextMenu, ContextMenuProps>((props: ContextMenuComponentProps, ref: React.Ref<IContextMenu>) => {
        ...
        const getContent: (item: MenuItemProps) => React.ReactNode = (item: MenuItemProps) => {
            ...
            return (<>
                {item.icon && <span className={['sf-menu-icon', typeof item.icon === 'string' ? item.icon : ''].filter(Boolean).join(' ')}>
            ...
            return openSubmenus.map(({ parentIndex, position, isVisible }: SubmenuType) => {
                ...
                return (
                    <ul
                        ...
                        className={'sf-menu-parent sf-ul'}
        ...
        return (<>
            {isOpen && createPortal(
                ...
                >
                    <ul
                        className="sf-control sf-contextmenu sf-menu-parent"
                        style={{
                            top: menuPosition.y,
                            left: menuPosition.x,
