...
export const useTimeIndicator: ({
    ...
    /**
     * Checks if the current time is within the schedule bounds.
     */
    const isTimeWithinScheduleBounds: () => boolean = useCallback((): boolean => {
    ...
    const updatePosition: () => void = useCallback((): void => {
        const now: Date = new Date();
        const withinBounds: boolean = isTimeWithinScheduleBounds();
    ...
    }, [calculatePosition, currentTime, isVisible, isWithinBounds, position, showTimeIndicator, isTimeWithinScheduleBounds]);
