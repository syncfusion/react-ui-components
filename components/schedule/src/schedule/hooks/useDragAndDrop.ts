...
import { EventModel, DragEventArgs } from '../types/schedule-types';
...
/**
 * Hook that enables drag and drop functionality for schedule events
 *
...
export function useDragAndDrop({ ref, data, containerProps }: UseDragAndDropParams): UseDragAndDropResult {
    const { timeScale, startHour, endHour, scheduleRef,
        eventSettings, showWeekend, workDays, onDragStart, onDrag, onDragStop } = useSchedulePropsContext();
    ...
    (newStartDate?: Date): void => {
        ...
        if (dragInfo.current.isMonthView || dragInfo.current.isAllDaySource) {
            ...
            const segments: ProcessedEventsData[] = EventService.processCloneEvent(scheduleRef, renderDates, eventInfo,
        ...
        const segments: ProcessedEventsData[] = EventService.processTimeSlotCloneEvent(scheduleRef, renderDates, eventInfo,
    ...
    const getDragInfoArgs: (evt: MouseEvent | TouchEvent) => DragEventArgs = (evt: MouseEvent | TouchEvent): DragEventArgs => {
        return {
            cancel: false,
            data: data,
            event: evt,
    ...
    const handleDragStart: (args: DragEventArgs) => void = (args: DragEventArgs) => {
        if (!dragInfo.current) { return; }
        dragStateRef.current = {};
        ...
        const startArgs: DragEventArgs = getDragInfoArgs(args.event);
    ...
    const getUpdateDragOffsets: (args: DragEventArgs) => void = (args: DragEventArgs): void => {
        try {
    ...
    const getAutoScroll: (args: DragEventArgs) => void = (args: DragEventArgs): void => {
            ...
            if (data) {
                ...
                if (isStepDragging.current && !info.isAllDaySource && !info.isMonthView) {
                    ...
                    if (containerRectSnap && coordinates.clientY != null && minutesPerPixel > 0) {
                        ...
                        const { scheduleStartMinutes } = DateService.getScheduleStartAndEndMinutes(startHour, endHour);
                        const dayStart: Date = new Date(cellDateAttr);
                        dayStart.setHours(0, 0, 0, 0);
                        computedMinutes = dayStart.getTime() + (scheduleStartMinutes + minutesFromTop) * 60000;
    ...
    const getDiffDuration: (args: DragEventArgs) => void = (args: DragEventArgs): void => {
    ...
    const handleDrag: (args: DragEventArgs) => void = (args: DragEventArgs) => {
        if (!dragInfo.current) { return; }
        getAutoScroll(args);
    ...
    const updateDropCellAndTimestamp: (args: DragEventArgs) => { cell: HTMLElement | null; finalDropTimestamp: number | null; } =
        (args: DragEventArgs): { cell: HTMLElement | null; finalDropTimestamp: number | null } => {
    ...
    const getUpdateEvent: (original: EventModel, newStartTime: Date, newEndTime: Date, args: DragEventArgs) => void =
        (original: EventModel, newStartTime: Date, newEndTime: Date, args: DragEventArgs) => {
            if (!dragInfo.current) { return; }
            const updatedEvent: EventModel = dragInfo.current.updateDatasource(original, newStartTime, newEndTime, scheduleRef);
            endDragCleanup();
            args.cancel = true;
    ...
    const handleDragStop: (args: DragEventArgs) => void = (args: DragEventArgs) => {
        if (!dragInfo.current) { return; }
        resetElementDragState();
        if (args.element) {
        ...
        if (isAllowDrop(cell)) {
            if (dragStateValuesRef.current.finalDragStartTime && dragStateValuesRef.current.finalDragEndTime) {
                getUpdateEvent(original, dragStateValuesRef.current.finalDragStartTime, dragStateValuesRef.current.finalDragEndTime, args);
                return;
            }
            if (cell) {
                if (finalDropTimestamp) {
                    const { newStartTime, newEndTime } = computeNewTimes(original, finalDropTimestamp);
    ...
    useDraggable(dragTargetRef, {
        ...
        dragStart: (args: DragEventArgs) => handleDragStart(args),
        drag: (args: DragEventArgs) => handleDrag(args),
        dragStop: (args: DragEventArgs) => handleDragStop(args)
    });
