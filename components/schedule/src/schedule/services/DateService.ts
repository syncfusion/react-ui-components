import { CalendarType, View } from '../types/enums';
import { ActiveViewProps } from '../types/internal-interface';
import { WorkHoursModel, EventModel } from '../types/schedule-types';
import { cldrData, formatDate, getDefaultDateObject, getValue } from '@syncfusion/react-base';
import { useScheduleLocalization } from '../../schedule/common/locale';
...
export class DateService {
     ...
     *
     * @param {Date} date - The date to check
     * @param {WorkHoursModel} workHours - The work hours configuration
     * @param {number[]} workDays - The work days (0 for Sunday, 1 for Monday, etc.)
     * @returns {boolean} True if the date is within work hours, false otherwise
     */
    static isWorkHour(date: Date, workHours: WorkHoursModel, workDays: number[]): boolean {
    ...
    static getScheduleStartAndEndMinutes(startHour: string, endHour: string): { scheduleStartMinutes: number; scheduleEndMinutes: number } {
        const startHourDate: Date = this.getStartEndHours(startHour);
        const endHourDate: Date = this.getStartEndHours(endHour);
        const scheduleStartMinutes: number = startHour === '00:00' ? 0 :
            startHourDate.getHours() * MINUTES_PER_HOUR + startHourDate.getMinutes();
        const scheduleEndMinutes: number = endHour === '24:00' ? MINUTES_PER_DAY :
    ...
    static setValidDate(date: any): Date {
    ...
    static getMonthRangeText(
        ...
        displayDate = this.setValidDate(displayDate);
        const modeOfCalendar: string = calendarMode.toLowerCase();
        if (!dateFormat) {
    ...
    static formatPopupDateRange(eventData: EventModel, locale: string): string {
        ...
        const { getString } = useScheduleLocalization(resolvedLocale);
        ...
        const endDateToFormat: Date = (isAllDay && this.isMidnight(end)) ? this.addDays(end, -1) : end;
        const endDateDetails: string = formatDate(endDateToFormat, { type: 'date', skeleton: 'long', locale: resolvedLocale });
