...
/**
 * Specifically merges ScheduleProps with deep merging for nested objects like eventSettings, workHours, timeScale
 *
 * @param {Partial<ScheduleProps>} defaultProps - The default schedule properties
 * @param {Partial<ScheduleProps>} userProps - The user provided properties
 * @returns {Partial<ScheduleProps>} The merged schedule properties with proper type safety
 */
export function mergeScheduleProps(
    defaultProps: Partial<ScheduleProps>,
    userProps: Partial<ScheduleProps>
): Partial<ScheduleProps> {
    return deepMerge(
        defaultProps as PlainObject,
        userProps as PlainObject
    ) as Partial<ScheduleProps>;
}
